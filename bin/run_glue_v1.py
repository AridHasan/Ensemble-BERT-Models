...
def main():
    ...
    send_example_telemetry("run_glue_v1", model_args, data_args)
    ...
    # Detecting last checkpoint. if --overwrite_output_dir is false, the trainer will start from the last saved checkpoint if available.
    ...
    if training_args.do_predict:
        if data_args.test_file is not None:
            ...
            ), "`test_file` should have the same extension as `train_file`."
    ...
    # try to have some nice defaults.
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        return {"f1": (preds == p.label_ids).astype(np.float32).mean().item()}
    ...
    if training_args.do_predict:
        ...
        metrics["pred_samples"] = min(max_pred_samples, len(predict_dataset))
